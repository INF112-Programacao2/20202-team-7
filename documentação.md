...
- Na ``main.cpp``, foi separada as funções que a secretária e a administração tem acesso. Como pode ser visto no trecho de codigo abaixo.
```c++
void Secretaria()
{
...
```
- Para que o usuário que não seja do administrativo tente acessar essa área sem autorização, foi determinado uma senha dentro do código.
```c++
...
```
- Essa senha pode ser alterada somente pelo desenvolvedor, para garantir mais confiabilidade ao programa.

- Na função main, foi posta o menu interativo, para que o usuário sinta a maior facilidade em utilizar o programa, sem maiores dificuldades, como representado no codigo abaixo:
```c++
int main()
{
...
```
- Nesse trecho de código é possivel ver os tratamentos feitos, para que caso o usuário digite algum numero que não esteja pré-configurado, ele receba notificações escritas, para que possa tentar novamente realizar o que pretendia.
- No demais, acredito que caso tenha alguma duvida, por favor comentar na documentação acerca de tal. 